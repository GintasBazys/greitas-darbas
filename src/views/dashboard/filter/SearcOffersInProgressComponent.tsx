...
import {setOffer, setReservedOffer} from "../../../features/offers/offersSlice";
...
const SearchOffersInProgressComponent = () => {
    ...
    let {
        ...
        db.collection("reservedOffers").where("user", "==", auth.currentUser?.uid).where("title", ">=", search).where("title", "<=", search+ "\uf8ff"), {
            limit: 5
        }
    );
    console.log(items);
